...
export function S01_Cover({ index, total }: { index: number; total: number }) {
  return (
    <Slide index={index} total={total} bare noFooter>
      <div
        className="relative flex h-[720px] w-[1280px] flex-col px-16 pt-12 pb-9"
        style={{
          background:
      ...
      >
        ...
        <div className="relative grid flex-1 grid-cols-[1.25fr_1fr] items-center gap-10">
          <div>
            ...
            <p className="mt-5 max-w-[560px] text-[15px] leading-relaxed text-[var(--body)]">
              Smartkarma unifies real-time independent research, specialist alternative
              data and direct access to analysts, augmented by{" "}
              <span className="font-semibold text-[var(--ai-pink)]">αSK</span>, an AI
              layer grounded in a decade of premium, curated research and designed to make
              human expertise more accessible, not less.
            </p>
          </div>
        ...
        <div className="relative grid grid-cols-5 items-center divide-x divide-[var(--hairline)] border-t border-[var(--hairline)] pt-4">
          {STATS.map((s, i) => (
            <div
              key={s.l}
              className={i === 0 ? "pop-in pr-6" : "pop-in px-6 last:pr-0"}
